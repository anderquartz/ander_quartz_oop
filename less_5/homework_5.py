...
class LengthValidator:
    ...
    def __call__(self, slovo):
        if len(slovo) < self.min_length or len(slovo) > self.max_length:
            raise ValueError('Длина не в диапазоне')
        return True


validator = LengthValidator(3, 10)
print(validator("python"))
# print(validator("hi")) ошибка
...
class HasText:
    def __init__(self, pod_text):
        self.pod_text = pod_text

    def __call__(self, text):
        return self.pod_text in text
        # if not self.pod_text in text: это я неправильно делал, оказалось всё проще
        #     return False
        # else:
        #     return True

assert HasText("Success")("Test passed: Success")
# assert HasText("Error")("All OK") Ошибка
...
class TestUser:
    ...
    def __repr__(self):
        return f"<TestUser if={self.id} name='{self.name}' email='{self.email}'>"
